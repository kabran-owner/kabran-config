...
- **cli:** Remove unified `kabran` CLI binary and all CLI commands
  - Removed: `kabran check`, `kabran test`, `kabran ci`, `kabran build`
  - Removed: `src/cli/*` and `src/core/config-loader.mjs`
  - Scripts remain available via `node node_modules/...` pattern
  - Use npm scripts directly instead of kabran CLI wrappers

- **telemetry:** Remove OpenTelemetry integration module
  - Removed: `@kabran-tecnologia/kabran-config/telemetry/*` exports
  - Removed: All `@opentelemetry/*` peer dependencies
  - Removed: `templates/telemetry/` directory

### Removed

- `kabran` CLI binary and all subcommands (check, test, ci, build)
- `config-loader.mjs` and dynamic configuration system
- All telemetry modules (config, frontend, edge, node, logger, shared)
- OpenTelemetry peer dependencies (12 packages)
- `--telemetry-env` setup option

### Changed

- Default runner type changed from `github` to `self-hosted`
- Validators now use inline defaults instead of config-loader
- `kabran-setup` always uses self-hosted CI workflow template
